  ...
  test("using 'use server' directive in pages", async () => {
    ...
    if (process.env.IS_TURBOPACK_TEST) {
      expect(next.normalizeTestDirContent(await session.getRedboxSource()))
       ...
       You're using the "use server" directive. That only works in a Server Component which is not supported in the pages/ directory. Read more: https://nextjs.org/docs/app/building-your-application/rendering/server-components
      ...
      expect(next.normalizeTestDirContent(await session.getRedboxSource()))
        .toMatchInlineSnapshot(`
       "./components/Comp.js
       Error:   x You're using the "use server" directive. That only works in a Server Component which is not supported in the pages/ directory. Read more: https://nextjs.org/docs/app/building-your-application/rendering/server-components
          ,-[1:1]
        1 | 'use server'
          : ^^^^^^^^^^^^
        2 | 
